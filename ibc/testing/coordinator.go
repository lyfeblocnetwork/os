...
const DefaultFeeAmt = int64(150_000_000_000_000_000) // 0.15 LYFEBLOC

var globalStartTime = time.Date(2020, 1, 2, 0, 0, 0, 0, time.UTC)

// NewCoordinator initializes Coordinator with N EVM TestChain's (Lyfebloc apps) and M Cosmos chains (Simulation Apps)
...
func SendMsgs(chain *ibctesting.TestChain, feeAmt int64, msgs ...sdk.Msg) (*sdk.Result, error) {
	...
	if lyfeblocChain, ok := chain.App.(*exampleapp.ExampleChain); ok {
		bondDenom, err = lyfeblocChain.StakingKeeper.BondDenom(chain.GetContext())
	} else {
