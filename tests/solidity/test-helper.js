...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network lyfebloc', 'run all tests using lyfebloc network')
    .example(
      '$0 --network lyfebloc --allowTests=test1,test2',
      'run only test1 and test2 using lyfebloc network'
    )
    .help('h')
    .alias('h', 'help')
    .describe('network', 'set which network to use: ganache|lyfebloc')
    .describe(
      'batch',
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'lyfebloc' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or lyfebloc')
    } else {
...
function loadTests (runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach((dirname) => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-lyfebloc']
...
function performTestSuite ({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-lyfebloc'
...
function setupNetwork ({ runConfig, timeout }) {
  if (runConfig.network !== 'lyfebloc') {
    // no need to start ganache. Truffle will start it
    return
  }

  // Spawn the lyfebloc process
