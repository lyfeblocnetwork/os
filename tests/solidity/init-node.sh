...
CHAINID="${CHAIN_ID:-lyfebloc_1775-1}"
...
# Set moniker and chain-id for Lyfebloc (Moniker can be anything, chain-id must be an integer)
osd init "$MONIKER" --chain-id "$CHAINID" --home "$CHAINDIR"

# Change parameter token denominations to alyfebloc
jq '.app_state["staking"]["params"]["bond_denom"]="alyfebloc"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="alyfebloc"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state["gov"]["params"]["min_deposit"][0]["denom"]="alyfebloc"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state["evm"]["params"]["evm_denom"]="alyfebloc"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state["mint"]["params"]["mint_denom"]="alyfebloc"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
...
osd genesis add-genesis-account "$(osd keys show "$VAL_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 100000000000000000000000000alyfebloc --keyring-backend "$KEYRING" --home "$CHAINDIR"
osd genesis add-genesis-account "$(osd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000alyfebloc --keyring-backend "$KEYRING" --home "$CHAINDIR"
osd genesis add-genesis-account "$(osd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000alyfebloc --keyring-backend "$KEYRING" --home "$CHAINDIR"
osd genesis add-genesis-account "$(osd keys show "$USER3_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000alyfebloc --keyring-backend "$KEYRING" --home "$CHAINDIR"
osd genesis add-genesis-account "$(osd keys show "$USER4_KEY" -a --keyring-backend "$KEYRING" --home "$CHAINDIR")" 1000000000000000000000alyfebloc --keyring-backend "$KEYRING" --home "$CHAINDIR"
...
osd genesis gentx "$VAL_KEY" 1000000000000000000000alyfebloc --gas-prices ${BASEFEE}alyfebloc --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
...
osd start "$TRACE" \
	--log_level $LOGLEVEL \
	--minimum-gas-prices=0.0001alyfebloc \
