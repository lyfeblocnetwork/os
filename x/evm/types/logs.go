...
func (tx TransactionLogs) Validate() error {
	if lyfebloctypes.IsEmptyHash(tx.Hash) {
...
func (log *Log) Validate() error {
	if err := lyfebloctypes.ValidateAddress(log.Address); err != nil {
		return fmt.Errorf("invalid log address %w", err)
	}
	if lyfebloctypes.IsEmptyHash(log.BlockHash) {
		return fmt.Errorf("block hash cannot be the empty %s", log.BlockHash)
	}
	if log.BlockNumber == 0 {
		return errors.New("block number cannot be zero")
	}
	if lyfebloctypes.IsEmptyHash(log.TxHash) {
		return fmt.Errorf("tx hash cannot be the empty %s", log.TxHash)
	}
	return nil
}

// ToEthereum returns the Ethereum type Log from an Lyfebloc proto compatible Log.
...
// LogsToEthereum casts the Lyfebloc logs to a slice of Ethereum Logs.
