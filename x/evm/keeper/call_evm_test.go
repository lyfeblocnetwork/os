...
func (suite *KeeperTestSuite) TestCallEVM() {
	LBTContract := common.HexToAddress(testconstants.LBTContractMainnet)
	testCases := []struct {
		name    string
		method  string
		expPass bool
	...
	for _, tc := range testCases {
		...
		res, err := suite.network.App.EVMKeeper.CallEVM(suite.network.GetContext(), erc20, types.ModuleAddress, LBTContract, false, tc.method, account)
		if tc.expPass {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.deploy {
				...
				res, err = suite.network.App.EVMKeeper.CallEVMWithData(suite.network.GetContext(), tc.from, &LBTContract, data, false)
			}
