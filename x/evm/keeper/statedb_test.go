...
func TestIterateContracts(t *testing.T) {
	...
	network.App.EVMKeeper.IterateContracts(network.GetContext(), func(addr common.Address, codeHash common.Hash) bool {
		// NOTE: we only care about the 2 contracts deployed above, not the ERC20 native precompile for the alyfebloc denomination
		if bytes.Equal(addr.Bytes(), common.HexToAddress(testconstants.LBTContractMainnet).Bytes()) {
			return false
		}
