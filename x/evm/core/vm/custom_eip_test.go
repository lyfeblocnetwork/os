	...
	testCases := []struct {
		...
		},
		{
			...
			map[string]func(*JumpTable){
				"lyfebloc_0": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				eips := GetActivatorsEipNames()
				require.ElementsMatch(t, append(eips_snapshot, "lyfebloc_0"), eips, "expected eips number to be equal")
			},
		},
		{
			"success - multiple new activators",
			map[string]func(*JumpTable){
				"lyfebloc_1": func(jt *JumpTable) {},
				"lyfebloc_2": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				...
				require.ElementsMatch(t, append(eips_snapshot, "lyfebloc_0", "lyfebloc_1", "lyfebloc_2"), eips, "expected eips number to be equal")
			},
		},
		{
