...
import (
	...
	"cosmossdk.io/math"
	"github.com/lyfeblocnetwork/os/x/erc20/keeper"
	...
	"github.com/lyfeblocnetwork/os/testutil"
	...
	"github.com/lyfeblocnetwork/os/contracts"
...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	ethsecpAddrLyfebloc := sdk.AccAddress(ethPk.PubKey().Address()).String()
	ethsecpAddrCosmos := sdk.MustBech32ifyAddressBytes(sdk.Bech32MainPrefix, ethsecpAddr)

	// Setup Cosmos <=> Lyfebloc IBC relayer
	sourceChannel := "channel-292"
	lyfeblocChannel := "channel-3"
	path := fmt.Sprintf("%s/%s", transfertypes.PortID, lyfeblocChannel)
	...
	testCases := []struct {
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(registeredDenom, "100", ethsecpAddrLyfebloc, ethsecpAddrCosmos, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, lyfeblocChannel, timeoutHeight, 0)
			},
			receiver:      secpAddr,
			disableERC20:  true,
			ackSuccess:    true,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(registeredDenom, "100", "lyfebloc", ethsecpAddrCosmos, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, lyfeblocChannel, timeoutHeight, 0)
			},
			receiver:      secpAddr,
			ackSuccess:    false,
			checkBalances: false,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(registeredDenom, "100", ethsecpAddrLyfebloc, "badbadhf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625", "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, lyfeblocChannel, timeoutHeight, 0)
			},
			receiver:      secpAddr,
		...
		},
		{
			...
			malleate: func() {
				...
				transfer := transfertypes.NewFungibleTokenPacketData(prefixedDenom, "100", secpAddrCosmos, ethsecpAddrLyfebloc, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, lyfeblocChannel, timeoutHeight, 0)
			},
			ackSuccess:    true,
		...
		},
		{
			...
			malleate: func() {
				transfer := transfertypes.NewFungibleTokenPacketData(erc20Denom, "100", secpAddrCosmos, ethsecpAddrLyfebloc, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, lyfeblocChannel, timeoutHeight, 0)
			},
			ackSuccess:    true,
		...
		},
		{
			...
			malleate: func() {
				...
				otherSecpAddrLyfebloc := sdk.AccAddress(pk1.PubKey().Address()).String()
				transfer := transfertypes.NewFungibleTokenPacketData(prefixedDenom, "500", otherSecpAddrLyfebloc, ethsecpAddrLyfebloc, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 1, transfertypes.PortID, sourceChannel, transfertypes.PortID, lyfeblocChannel, timeoutHeight, 0)
			},
			ackSuccess: true,
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.network.App.IBCKeeper.ChannelKeeper.SetChannel(ctx, transfertypes.PortID, lyfeblocChannel, channel)

			// Set Next Sequence Send
			suite.network.App.IBCKeeper.ChannelKeeper.SetNextSequenceSend(ctx, transfertypes.PortID, lyfeblocChannel, 1)
			...
			// Fund receiver account with LYFEBLOC, ERC20 coins and IBC vouchers
...
func (suite *KeeperTestSuite) TestConvertCoinToERC20FromPacket() {
	var ctx sdk.Context
	senderAddr := "lyfebloc1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v"
