...
import (
	...
	rpctypes "github.com/lyfeblocnetwork/os/rpc/types"
...
var _ lyfebloctypes.EVMTxIndexer = &KVIndexer{}
...
func (kv *KVIndexer) IndexBlock(block *cmttypes.Block, txResults []*abci.ExecTxResult) error {
	...
	for txIndex, tx := range block.Txs {
		...
		for msgIndex, msg := range tx.GetMsgs() {
			...
			txResult := lyfebloctypes.TxResult{
				Height:     height,
...
func (kv *KVIndexer) GetByTxHash(hash common.Hash) (*lyfebloctypes.TxResult, error) {
	...
	var txKey lyfebloctypes.TxResult
...
func (kv *KVIndexer) GetByBlockAndIndex(blockNumber int64, txIndex int32) (*lyfebloctypes.TxResult, error) {
...
func saveTxResult(codec codec.Codec, batch dbm.Batch, txHash common.Hash, txResult *lyfebloctypes.TxResult) error {
