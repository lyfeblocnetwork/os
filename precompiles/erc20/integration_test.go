...
import (
	...
	auth "github.com/lyfeblocnetwork/os/precompiles/authorization"
	"github.com/lyfeblocnetwork/os/precompiles/erc20"
	"github.com/lyfeblocnetwork/os/precompiles/erc20/testdata"
	...
	"github.com/lyfeblocnetwork/os/testutil/integration/os/keyring"
	"github.com/lyfeblocnetwork/os/testutil/integration/os/network"
	integrationutils "github.com/lyfeblocnetwork/os/testutil/integration/os/utils"
...
var _ = Describe("ERC20 Extension -", func() {
	...
	BeforeEach(func() {
		...
		Expect(common.HexToAddress(erc20Params.NativePrecompiles[0])).To(Equal(common.HexToAddress(testconstants.LBTContractMainnet)))
	...
	Context("basic functionality -", func() {
		...
		When("calling reverter contract", func() {
			Context("in a direct call to the LBT contract", func() {
				var (
				...
				DescribeTable("it should revert token transfer from the LBT contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					amountToSend := big.NewInt(100)
				...
				It("it should send token transfer and send from LBT contract", func() {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
				...
				DescribeTable("it should revert token transfer and send from LBT contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					balRes, err := is.handler.GetBalanceFromBank(receiver.Bytes(), is.bondDenom)
					Expect(err).To(BeNil())
		...
		When("querying allowance", func() {
			...
			When("querying the allowance for the own address", func() {
				...
				//
				// For reference see this comment: https://github.com/lyfeblocnetwork/lyfebloc/pull/2088#discussion_r1407646217
...
var _ = Describe("ERC20 Extension migration Flows -", func() {
	...
	When("using Lyfebloc (not LBT) in smart contracts", func() {
		It("should be using straight Lyfebloc for sending funds in smart contracts", func() {
			Skip("will be addressed in follow-up PR")
