...
import (
	...
	lyfeblocutil "github.com/lyfeblocnetwork/os/testutil/constants"
	...
	compiledcontracts "github.com/lyfeblocnetwork/os/contracts"
	"github.com/lyfeblocnetwork/os/crypto/ethsecp256k1"
	"github.com/lyfeblocnetwork/os/precompiles/authorization"
	cmn "github.com/lyfeblocnetwork/os/precompiles/common"
	"github.com/lyfeblocnetwork/os/precompiles/distribution"
	"github.com/lyfeblocnetwork/os/precompiles/staking"
	"github.com/lyfeblocnetwork/os/precompiles/staking/testdata"
	"github.com/lyfeblocnetwork/os/precompiles/testutil"
	"github.com/lyfeblocnetwork/os/precompiles/testutil/contracts"
	"github.com/lyfeblocnetwork/os/testutil/integration/os/factory"
	testutils "github.com/lyfeblocnetwork/os/testutil/integration/os/utils"
	testutiltx "github.com/lyfeblocnetwork/os/testutil/tx"
...
var _ = Describe("Calling staking precompile directly", func() {
	var (
		...
		oneE18Coin = sdk.NewCoin(lyfeblocutil.ExampleAttoDenom, math.NewInt(1e18))
		// twoE18Coin is a sdk.Coin with an amount of 2e18 in the test suite's bonding denomination
		twoE18Coin = sdk.NewCoin(lyfeblocutil.ExampleAttoDenom, math.NewInt(2e18))
	)
	...
	Describe("Execute approve transaction", func() {
		...
		It("should approve the undelegate method with 1 lyfebloc", func() {
			granter := s.keyring.GetKey(0)
			grantee := s.keyring.GetKey(1)
		...
		It("should approve the redelegate method with 2 lyfebloc", func() {
			granter := s.keyring.GetKey(0)
			grantee := s.keyring.GetKey(1)
		...
		It("should approve the cancel unbonding delegation method with 1 lyfebloc", func() {
			granter := s.keyring.GetKey(0)
			grantee := s.keyring.GetKey(1)
	...
	Describe("Execute increase allowance transaction", func() {
		...
		It("Should increase the allowance of the delegate method with 1 lyfebloc", func() {
			granter := s.keyring.GetKey(0)
			grantee := s.keyring.GetKey(1)
	...
	Describe("Execute decrease allowance transaction", func() {
		...
		It("Should decrease the allowance of the delegate method with 1 lyfebloc", func() {
			granteeAddr := s.precompile.Address()
			granter := s.keyring.GetKey(0)
	...
	Describe("Redelegations queries", func() {
		...
		It("should return an empty array if no redelegation is found for the given source validator", func() {
			...
			// possible (see https://github.com/lyfebloc/cosmos-sdk/blob/e773cf768844c87245d0c737cda1893a2819dd89/x/staking/keeper/querier.go#L361-L373):
			//
...
var _ = Describe("Calling staking precompile via Solidity", Ordered, func() {
	...
	Context("querying delegation", func() {
		...
		It("which does not exist should return an empty delegation", func() {
			...
			Expect(delOut.Balance.Denom).To(Equal(lyfeblocutil.ExampleAttoDenom), "expected a different delegation balance")
		})
		...
		It("which exists should return the delegation", func() {
			...
			Expect(delOut.Balance).To(Equal(
				cmn.Coin{Denom: lyfeblocutil.ExampleAttoDenom, Amount: big.NewInt(1e18)}),
