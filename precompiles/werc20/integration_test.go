...
import (
	...
	"github.com/lyfeblocnetwork/os/precompiles/werc20"
	"github.com/lyfeblocnetwork/os/precompiles/werc20/testdata"
...
func TestPrecompileIntegrationTestSuite(t *testing.T) {
	...
	RunSpecs(t, "LBT precompile test suite")
}
...
var _ = When("a user interact with the LBT precompiled contract", func() {
	var (
	...
	BeforeEach(func() {
		...
		is.precompileAddrHex = network.GetLBTContractHex(is.network.GetChainID())
		...
		// Check that LBT is part of the native precompiles.
		...
		Expect(erc20Params.NativePrecompiles).To(
			...
			"expected LBT to be in the native precompiles",
		)
		...
		// Check that LBT is registered in the token pairs map.
		...
		Expect(found).To(BeTrue(), "expected LBT precompile to be registered in the tokens map")
		...
		revertCallerContract, err := testdata.LoadLBT9TestCaller()
	...
	Context("calling an erc20 method", func() {
		...
		When("querying information", func() {
			...
			It("should return the correct name", func() {
				...
				Expect(err).ToNot(HaveOccurred(), "failed to unpack result")
				Expect(name).To(ContainSubstring("Lyfebloc"), "expected different name")
			})
			...
			It("should return the correct symbol", func() {
				...
				Expect(err).ToNot(HaveOccurred(), "failed to unpack result")
				Expect(symbol).To(ContainSubstring("LYFEBLOC"), "expected different symbol")
			})
