...
import (
	...
	"github.com/lyfeblocnetwork/os/precompiles/bank/testdata"
...
func (is *IntegrationTestSuite) SetupTest() {
	...
	is.lyfeblocAddr = common.HexToAddress(tokenPair.Erc20Address)
...
var _ = Describe("Bank Extension -", func() {
	var (
		...
		lyfeblocTotalSupply, _ = new(big.Int).SetString("200003000000000000000000", 10)
	...
	Context("Direct precompile queries", func() {
		...
		Context("totalSupply query", func() {
			It("should return the correct total supply", func() {
				...
				Expect(balances[0].Amount.String()).To(Equal(lyfeblocTotalSupply.String()))
				Expect(balances[1].Amount.String()).To(Equal(xmplTotalSupply.String()))
			})
		})

		Context("supplyOf query", func() {
			It("should return the supply of Lyfebloc", func() {
				queryArgs, supplyArgs := getTxAndCallArgs(directCall, contractData, bank.SupplyOfMethod, is.lyfeblocAddr)
				_, ethRes, err := is.factory.CallContractAndCheckLogs(sender.Priv, queryArgs, supplyArgs, passCheck)
				Expect(err).ToNot(HaveOccurred(), "unexpected result calling contract")
				...
				Expect(out[0].(*big.Int).String()).To(Equal(lyfeblocTotalSupply.String()))
			})
	...
	Context("Calls from a contract", func() {
		...
		Context("supplyOf query", func() {
			It("should return the supply of Lyfebloc", func() {
				queryArgs, supplyArgs := getTxAndCallArgs(contractCall, contractData, SupplyOfFunction, is.lyfeblocAddr)
				_, ethRes, err := is.factory.CallContractAndCheckLogs(sender.Priv, queryArgs, supplyArgs, passCheck)
				Expect(err).ToNot(HaveOccurred(), "unexpected result calling contract")
