...
import (
	...
	"github.com/lyfeblocnetwork/os/cmd/config"
	"github.com/lyfeblocnetwork/os/indexer"
	ethdebug "github.com/lyfeblocnetwork/os/rpc/namespaces/ethereum/debug"
	lyfeblocserverconfig "github.com/lyfeblocnetwork/os/server/config"
	srvflags "github.com/lyfeblocnetwork/os/server/flags"
...
`,
	...
	cmd.Flags().String(server.FlagMinGasPrices, "", "Minimum gas prices to accept for transactions; Any fee in a tx must meet this minimum (e.g. 20000000000alyfebloc)") //nolint:lll
	...
	cmd.Flags().Bool(srvflags.GRPCEnable, lyfeblocserverconfig.DefaultGRPCEnable, "Define if the gRPC server should be enabled")
	cmd.Flags().String(srvflags.GRPCAddress, serverconfig.DefaultGRPCAddress, "the gRPC server address to listen on")
	cmd.Flags().Bool(srvflags.GRPCWebEnable, lyfeblocserverconfig.DefaultGRPCWebEnable, "Define if the gRPC-Web server should be enabled. (Note: gRPC must also be enabled.)")
	cmd.Flags().String(srvflags.GRPCWebAddress, lyfeblocserverconfig.DefaultGRPCAddress, "The gRPC-Web server address to listen on")

	cmd.Flags().Bool(srvflags.RPCEnable, lyfeblocserverconfig.DefaultAPIEnable, "Defines if Cosmos-sdk REST server should be enabled")
	cmd.Flags().Bool(srvflags.EnabledUnsafeCors, false, "Defines if CORS should be enabled (unsafe - use it at your own risk)")

	cmd.Flags().Bool(srvflags.JSONRPCEnable, lyfeblocserverconfig.DefaultJSONRPCEnable, "Define if the JSON-RPC server should be enabled")
	cmd.Flags().StringSlice(srvflags.JSONRPCAPI, lyfeblocserverconfig.GetDefaultAPINamespaces(), "Defines a list of JSON-RPC namespaces that should be enabled")
	cmd.Flags().String(srvflags.JSONRPCAddress, lyfeblocserverconfig.DefaultJSONRPCAddress, "the JSON-RPC server address to listen on")
	cmd.Flags().String(srvflags.JSONWsAddress, lyfeblocserverconfig.DefaultJSONRPCWsAddress, "the JSON-RPC WS server address to listen on")
	cmd.Flags().Uint64(srvflags.JSONRPCGasCap, lyfeblocserverconfig.DefaultGasCap, "Sets a cap on gas that can be used in eth_call/estimateGas unit is alyfebloc (0=infinite)")                        //nolint:lll
	cmd.Flags().Bool(srvflags.JSONRPCAllowInsecureUnlock, lyfeblocserverconfig.DefaultJSONRPCAllowInsecureUnlock, "Allow insecure account unlocking when account-related RPCs are exposed by http") //nolint:lll
	cmd.Flags().Float64(srvflags.JSONRPCTxFeeCap, lyfeblocserverconfig.DefaultTxFeeCap, "Sets a cap on transaction fee that can be sent via the RPC APIs (1 = default 1 lyfebloc)")                    //nolint:lll
	cmd.Flags().Int32(srvflags.JSONRPCFilterCap, lyfeblocserverconfig.DefaultFilterCap, "Sets the global cap for total number of filters that can be created")
	cmd.Flags().Duration(srvflags.JSONRPCEVMTimeout, lyfeblocserverconfig.DefaultEVMTimeout, "Sets a timeout used for eth_call (0=infinite)")
	cmd.Flags().Duration(srvflags.JSONRPCHTTPTimeout, lyfeblocserverconfig.DefaultHTTPTimeout, "Sets a read/write timeout for json-rpc http server (0=infinite)")
	cmd.Flags().Duration(srvflags.JSONRPCHTTPIdleTimeout, lyfeblocserverconfig.DefaultHTTPIdleTimeout, "Sets a idle timeout for json-rpc http server (0=infinite)")
	cmd.Flags().Bool(srvflags.JSONRPCAllowUnprotectedTxs, lyfeblocserverconfig.DefaultAllowUnprotectedTxs, "Allow for unprotected (non EIP155 signed) transactions to be submitted via the node's RPC when the global parameter is disabled") //nolint:lll
	cmd.Flags().Int32(srvflags.JSONRPCLogsCap, lyfeblocserverconfig.DefaultLogsCap, "Sets the max number of results can be returned from single `eth_getLogs` query")
	cmd.Flags().Int32(srvflags.JSONRPCBlockRangeCap, lyfeblocserverconfig.DefaultBlockRangeCap, "Sets the max block range allowed for `eth_getLogs` query")
	cmd.Flags().Int(srvflags.JSONRPCMaxOpenConnections, lyfeblocserverconfig.DefaultMaxOpenConnections, "Sets the maximum number of simultaneous connections for the server listener") //nolint:lll
	...
	cmd.Flags().String(srvflags.EVMTracer, lyfeblocserverconfig.DefaultEVMTracer, "the EVM tracer type to collect execution traces from the EVM transaction execution (json|struct|access_list|markdown)") //nolint:lll
	cmd.Flags().Uint64(srvflags.EVMMaxTxGasWanted, lyfeblocserverconfig.DefaultMaxTxGasWanted, "the gas wanted for each eth tx returned in ante handler in check tx mode")                                 //nolint:lll
...
func startStandAlone(svrCtx *server.Context, opts StartOptions) error {
	...
	config, err := lyfeblocserverconfig.GetConfig(svrCtx.Viper)
	if err != nil {
...
func startInProcess(svrCtx *server.Context, clientCtx client.Context, opts StartOptions) (err error) {
	...
	var idxer lyfebloctypes.EVMTxIndexer
...
func startTelemetry(cfg lyfeblocserverconfig.Config) (*telemetry.Metrics, error) {
...
func startJSONRPCServer(
	...
	clientCtx client.Context,
	g *errgroup.Group,
	config lyfeblocserverconfig.Config,
	genDocProvider node.GenesisDocProvider,
	cmtRPCAddr string,
	idxer lyfebloctypes.EVMTxIndexer,
