...
import (
	...
	chaincmd "github.com/lyfeblocnetwork/os/example_chain/osd/cmd"
	"github.com/lyfeblocnetwork/os/server/config"
...
type Config struct {
	...
	GenesisState      lyfebloctypes.GenesisState // custom gensis state to provide
...
func DefaultConfig() Config {
	chainID := fmt.Sprintf("lyfebloc_%d-1", cmtrand.Int63n(9999999999999)+1)
	...
	cfg := Config{
		...
		AccountTokens:     sdk.TokensFromConsensusPower(1000000000000000000, lyfebloctypes.AttoPowerReduction),
		StakingTokens:     sdk.TokensFromConsensusPower(500000000000000000, lyfebloctypes.AttoPowerReduction),
		BondedTokens:      sdk.TokensFromConsensusPower(100000000000000000, lyfebloctypes.AttoPowerReduction),
...
// NewAppConstructor returns a new Lyfebloc AppConstructor
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	if !lyfebloctypes.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "lyfeblocd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "lyfebloccli")
