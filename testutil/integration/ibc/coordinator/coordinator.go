...
import (
	...
	lyfeblocibc "github.com/lyfeblocnetwork/os/ibc/testing"
	"github.com/lyfeblocnetwork/os/testutil/integration/common/network"
	ibcchain "github.com/lyfeblocnetwork/os/testutil/integration/ibc/chain"
)
...
type Coordinator interface {
	...
	GetPath(a, b string) *lyfeblocibc.Path
...
// GetPath returns the transfer path for the chain ids 'a' and 'b'
func (c *IntegrationCoordinator) GetPath(a, b string) *lyfeblocibc.Path {
	chainA := c.coord.GetChain(a)
	chainB := c.coord.GetChain(b)

	return lyfeblocibc.NewTransferPath(chainA, chainB)
}
...
func (c *IntegrationCoordinator) Setup(a, b string) IBCConnection {
	path := c.GetPath(a, b)
	lyfeblocibc.SetupPath(c.coord, path)
