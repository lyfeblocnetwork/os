...
import (
	...
	"github.com/lyfeblocnetwork/os/example_chain/eips"
	"github.com/lyfeblocnetwork/os/example_chain/eips/testdata"
...
func TestIPs(t *testing.T) {
	...
	RunSpecs(t, "LyfeblocIPs Suite")
}

var _ = Describe("Improvement proposal lyfebloc_0 - ", Ordered, func() {
	var (
		in network.Network
	...
	// Multiplier used to modify the opcodes associated with lyfebloc_0 IP.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have IP lyfebloc_0 in evm params")
	})

	It("should change CREATE opcode constant gas after enabling lyfebloc_0 IP", func() {
...
var _ = Describe("Improvement proposal lyfebloc_1 - ", Ordered, func() {
	var (
		in network.Network
		tf factory.TxFactory
		gh grpc.Handler
	...
	// Multiplier used to modify the opcodes associated with lyfebloc_1.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have ip lyfebloc_1 in evm params")
	})
...
var _ = Describe("Improvement proposal lyfebloc_2 - ", Ordered, func() {
	var (
		in network.Network
		tf factory.TxFactory
		gh grpc.Handler
	...
	// Constant gas used to modify the opcodes associated with lyfebloc_2.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have ip lyfebloc_2 in evm params")
	})
