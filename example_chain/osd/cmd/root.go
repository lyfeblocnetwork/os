...
import (
	...
	lyfebloccmd "github.com/lyfeblocnetwork/os/client"
	lyfebloccmdconfig "github.com/lyfeblocnetwork/os/cmd/config"
	lyfeblockeyring "github.com/lyfeblocnetwork/os/crypto/keyring"
	"github.com/lyfeblocnetwork/os/example_chain"
	cmdcfg "github.com/lyfeblocnetwork/os/example_chain/osd/config"
	lyfeblocserver "github.com/lyfeblocnetwork/os/server"
...
func NewRootCmd() *cobra.Command {
	...
	initClientCtx := client.Context{}.
		...
		// Lyfebloc specific setup
		WithKeyringOptions(lyfeblockeyring.Option()).
		WithLedgerHasProtobuf(true)

	rootCmd := &cobra.Command{
		Use:   "osd",
		Short: "exemplary Lyfebloc app",
...
func InitAppConfig(denom string) (string, interface{}) {
	type CustomAppConfig struct {
		...
		EVM     lyfeblocserverconfig.EVMConfig
		JSONRPC lyfeblocserverconfig.JSONRPCConfig
		TLS     lyfeblocserverconfig.TLSConfig
	}
	...
	customAppConfig := CustomAppConfig{
		Config:  *srvCfg,
		EVM:     *lyfeblocserverconfig.DefaultEVMConfig(),
		JSONRPC: *lyfeblocserverconfig.DefaultJSONRPCConfig(),
		TLS:     *lyfeblocserverconfig.DefaultTLSConfig(),
	}

	customAppTemplate := serverconfig.DefaultConfigTemplate +
		lyfeblocserverconfig.DefaultEVMConfigTemplate
...
func initRootCmd(rootCmd *cobra.Command, osApp *example_chain.ExampleChain) {
	...
	// add Lyfebloc' flavored TM commands to start server, etc.
	lyfeblocserver.AddCommands(
		rootCmd,
		lyfeblocserver.NewDefaultStartOptions(newApp, example_chain.DefaultNodeHome),
		appExport,
		addModuleInitFlags,
	)

	// add Lyfebloc key commands
	rootCmd.AddCommand(
		lyfebloccmd.KeyCommands(example_chain.DefaultNodeHome, true),
	)
...
func newApp(
	...
	if chainID == "" {
		chainID, err = lyfebloccmdconfig.GetChainIDFromHome(homeDir)
		if err != nil {
			panic(err)
		}
	}
	...
	// Set up the required mempool and ABCI proposal handlers for Lyfebloc
