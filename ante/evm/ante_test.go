	...
	testCases := []struct {
		...
		},
		{
			...
			func() sdk.Tx {
				...
				gas := uint64(200000)
				amount := sdk.NewCoins(sdk.NewCoin(testconstants.ExampleAttoDenom, sdkmath.NewInt(100*int64(gas)))) //#nosec G115
				txBuilder, err := suite.CreateTestEIP712TxBuilderMsgSend(from, privKey, "lyfebloc_1775-1", gas, amount)
				suite.Require().NoError(err)
				return txBuilder.GetTx()
			}, false, false, false,
		},
		...
		},
		{
			...
			func() sdk.Tx {
				...
				gas := uint64(200000)
				amount := sdk.NewCoins(sdk.NewCoin(testconstants.ExampleAttoDenom, sdkmath.NewInt(100*int64(gas)))) //#nosec G115
				txBuilder, err := suite.CreateTestEIP712TxBuilderMsgSend(from, privKey, "lyfebloc_9005-1", gas, amount)
				suite.Require().NoError(err)
				return txBuilder.GetTx()
			}, false, false, false,
		},
